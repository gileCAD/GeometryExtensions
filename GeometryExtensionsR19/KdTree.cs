...
namespace Gile.AutoCAD.R19.Geometry
{
    ...
    public class KdTree<T>
    {
        ...
        private TreeNode<T> Create(T[] items, int depth)
        {
            ...
            var median = SelectMedian(items, (x, y) => getPosition(x)[d].CompareTo(getPosition(y)[d]));
        ...
        private T SelectMedian(T[] items, Comparison<T> compare)
        {
            int l = items.Length;
            int k = l / 2;
            if (items == null || l == 0)
